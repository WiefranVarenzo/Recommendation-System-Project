...
# Ambil rekomendasi produk berdasarkan LSA
recs = get_lsa_recommendations(product_id=example_id, top_n=5)

# Tampilkan hasil rekomendasi produk dalam format tabel
print("\nRekomendasi Produk:\n")
print(recs.to_string(index=False))

# Ambil top-n kata dari vektor TF-IDF produk tersebut
row = tfidf_matrix[example_id].toarray().flatten()
top_n_words = np.argsort(row)[::-1][:10]
top_words = [feature_names[i] for i in top_n_words]

# Tampilkan top words dengan format yang konsisten
...
# **7. Kesimpulan Umum/Conclusion**

Hasil evaluasi menunjukkan bahwa kedua pendekatan — **TF-IDF + Nearest Neighbors** dan **LSA (Latent Semantic Analysis)** — mampu memberikan **rekomendasi produk yang relevan dan konsisten** berdasarkan deskripsi teks. Keduanya berhasil mengidentifikasi kemiripan antara produk berdasarkan fitur-fitur penting seperti **bahan**, **gaya**, dan **nuansa deskriptif**.

Namun, terdapat beberapa perbedaan kunci yang mengarah pada kesimpulan lebih dalam:

1. **Ketepatan Semantik dan Generalisasi:**

   * **LSA** unggul dalam menangkap **makna konseptual dan semantik** dari kata-kata, bahkan jika tidak identik secara literal, karena menggunakan reduksi dimensi dengan SVD.
   * **TF-IDF** bekerja sangat baik untuk kata-kata eksplisit dan unik, tetapi kurang fleksibel terhadap sinonim atau konteks yang lebih dalam.

2. **Konsistensi dan Validasi:**

   * Hasil rekomendasi dari **kedua metode sangat konsisten**, yang memvalidasi kekuatan dan keandalan sistem berbasis konten ini.
   * LSA memperkuat dan menyempurnakan hasil TF-IDF, khususnya dalam menangkap perbedaan halus dan nuansa bahasa.

3. **Potensi Pengembangan:**

   * **TF-IDF** dapat ditingkatkan dengan preprocessing lanjutan atau digunakan sebagai baseline.
   * **LSA** lebih cocok untuk skala besar dan kompleksitas tinggi, serta sangat potensial jika dikombinasikan dengan data pengguna dalam sistem **hybrid recommendation** (misalnya, menggabungkan preferensi pengguna dengan pemahaman semantik produk).

### **Kesimpulan Akhir:**

Sistem rekomendasi berbasis konten yang dibangun menggunakan TF-IDF dan LSA terbukti **efektif, relevan, dan andal**. Pendekatan LSA memberikan **lapisan semantik tambahan** yang penting untuk memahami konteks dan variasi bahasa, sehingga menjadikannya pilihan lebih kuat dalam lingkungan nyata yang dinamis. Penggabungan keduanya, serta integrasi data perilaku pengguna, akan memberikan fondasi kuat untuk **sistem rekomendasi cerdas dan personalisasi tingkat lanjut**.
"""